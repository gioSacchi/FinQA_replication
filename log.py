...
model = "roberta-large-gpt3-text-only"
log_file = "output\\"+model+"-"+type+".txt"
batch_size = None
report_interval = None

# train_data_size = 6251  # Standard
# train_data_size = 27718
# train_data_size = 37500  # GPT3
train_data_size = 13538  # GPT3 text only
...
print("Average time per iteration: ", round(
    total_time/len(iterations), 2), " seconds")
...
if type == "retriever":
    ...
    print("Best top 3 validation",  max_index_3 + 1,
          ":", validation_accuracy_3[max_index_3])

    max_index_5 = validation_accuracy_5.index(max(validation_accuracy_5))
    print("Best top 5 validation",  max_index_5 + 1,
          ":", validation_accuracy_5[max_index_5])


print()
# Last validation accuracy index
if type == "retriever":
    print("Last top 3 validation",  len(validation_accuracy_3),
          ":", validation_accuracy_3[-1])
    print("Last top 5 validation",  len(validation_accuracy_5),
          ":", validation_accuracy_5[-1])

# Generator
# Max validation accuracy index
if type == "generator":
    max_index_exe = exe_accuracy.index(max(exe_accuracy))
    print("Best exe accuracy",  max_index_exe +
          1, ":", exe_accuracy[max_index_exe])

    max_index_prog = prog_accuracy.index(max(prog_accuracy))
    print("Best prog accuracy",  max_index_prog +
          1, ":", prog_accuracy[max_index_prog])
...
ax1.set_title("%s %s - loss (%s batch size) %s epochs" %
              (type, model, batch_size, epochs))
...
if type == "retriever":
    ax2.plot([x * report_interval for x in range(1,
             len(validation_accuracy_3) + 1)], validation_accuracy_3, label="Top 3")
    ax2.plot([x * report_interval for x in range(1,
             len(validation_accuracy_5) + 1)], validation_accuracy_5, label="Top 5")
    ax2.set_title("Retriever %s - validation accuracy (%s batch size) %s epochs" %
...
if type == "generator":
    ax2.plot([x * report_interval for x in range(1,
             len(exe_accuracy) + 1)], exe_accuracy, label="Exe")
    ax2.plot([x * report_interval for x in range(1,
             len(exe_accuracy) + 1)], prog_accuracy, label="Prog")
    ax2.set_title("%s %s - accuracy (%s batch size) %s epochs" %
                  (type, model, batch_size, epochs))
    ax2.set_xlabel("Iterations")
